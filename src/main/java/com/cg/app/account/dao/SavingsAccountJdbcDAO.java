...
public class SavingsAccountJdbcDAO implements SavingsAccountDAO {
	...
	public SavingsAccount getAccountById(int accountNumber)
		...
		SavingsAccount account = template.queryForObject("SELECT * FROM account where account_id=?",
	...
	public void updateBalance(int accountNumber, double currentBalance) throws ClassNotFoundException, SQLException {
		template.update("UPDATE ACCOUNT SET account_bal=? where account_id=?",
	...
	public SavingsAccount getAccountByName(String accountHolderName)
		...
		SavingsAccount account = template.queryForObject("SELECT * FROM account where account_hn=?",
	...
	public List<SavingsAccount> getAccountsBetweenTheSalary(double minimum, double maximum)
		...
		List<SavingsAccount> list = template.query("SELECT * FROM account where account_bal BETWEEN ? AND ?",
	...
	public boolean updateAccountType(SavingsAccount account) throws SQLException, ClassNotFoundException {
		...
		return template.update("UPDATE ACCOUNT SET account_hn=?,salary=? where account_id=?",
				new Object[] { account.getBankAccount().getAccountHolderName(), account.isSalary(),
						account.getBankAccount().getAccountNumber() }) > 0;
	}
